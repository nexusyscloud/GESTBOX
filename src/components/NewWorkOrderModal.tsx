...
const NewWorkOrderModal: React.FC<NewWorkOrderModalProps> = ({
  ...
  useEffect(() => {
    if (formData.cliente_id) {
      const clienteVehiculos = vehiculos.filter(v => v.cliente_id === formData.cliente_id);
      setFilteredVehiculos(clienteVehiculos);
      if (!clienteVehiculos.find(v => v.id === formData.vehiculo_id)) {
  ...
  const fetchData = async () => {
    ...
    try {
      const [clientesData, vehiculosData, nextNumber] = await Promise.all([
        fetchClientes(),
        fetchVehiculos(),
  ...
  const handleCreateVehicle = async () => {
    ...
    if (insertError) {
      setError('Error al crear vehiculo: ' + insertError.message);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Creando orden con datos:', {
      ...
      vehiculo_id: orderData.vehiculo_id,
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  <p className="mt-1 text-xs text-gray-500">Generado automaticamente</p>
                </div>
              ...
              <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                    ...
                    >
                      ...
                      {filteredVehiculos.map(vehiculo => (
                        <option key={vehiculo.id} value={vehiculo.id}>
                          {[vehiculo.placa, vehiculo.marca, vehiculo.modelo].filter(Boolean).join(' - ') || 'Sin datos'}
                        </option>
                      ))}
                    </select>
                    {filteredVehiculos.length === 0 && (
                      <p className="mt-2 text-sm text-amber-600">
                        Este cliente no tiene vehiculos registrados. Crea uno nuevo.
                      </p>
                    )}
                  </>
                )}
                ...
                {formData.vehiculo_id && (
                  <p className="mt-2 text-xs text-green-600">
                    Vehiculo seleccionado: ID {formData.vehiculo_id.slice(0, 8)}...
                  </p>
                )}
              </div>
